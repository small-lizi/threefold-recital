	...
	if event is InputEventKey and event.pressed and not is_dragging:
		match event.keycode:
			KEY_1:
				stop_movement()
				play_animation("idel")
				set_node_visibility(["desk", "sofa","lamp"], false)
			KEY_2:
				set_node_visibility(["desk", "sofa","lamp"], false)
				toggle_movement("run")
			KEY_3:
				set_node_visibility(["desk", "sofa","lamp"], false)
				toggle_movement("walk")
			KEY_4:
				stop_movement()
				play_animation("disco")
				set_node_visibility(["desk"], true)
				set_node_visibility(["sofa","lamp"], false)
			KEY_5:
				stop_movement()
				play_animation("disco2")
				set_node_visibility(["desk"], true)
				set_node_visibility(["sofa","lamp"], false)
			KEY_6:
				stop_movement()
				play_animation("lie")
				set_node_visibility(["sofa","lamp"], true)
				set_node_visibility(["desk"], false)
