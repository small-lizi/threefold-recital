	...
	if event is InputEventKey and event.pressed and not is_dragging:
		match event.keycode:
			KEY_1:
				stop_movement()
				play_animation("idel")
				set_node_visibility(["chair"], false)
				set_node_visibility(["bed"], false)
			KEY_2:
				set_node_visibility(["chair"], false)
				set_node_visibility(["bed"], false)
				toggle_movement("run")
			...
			KEY_8:
				stop_movement()
				play_animation("struggle")
				set_node_visibility(["chair"], true)
				set_node_visibility(["bed"], false)
			KEY_9:
				set_node_visibility(["chair"], false)
				set_node_visibility(["bed"], false)
				toggle_movement("shoulder")  # 扛着东西时也会移动
...
func _on_animation_finished():
	if animated_sprite.animation in ["finger", "sleep", "look"]:
		play_animation("idel")
