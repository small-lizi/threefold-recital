...
@export var ai_interval: float = 999999  # AI行为间隔时间(秒)
...
# 道具节点列表
var prop_nodes = ["zohar", "bulu", "lang"]

func _ready():
	animated_sprite.play("idel")
	set_node_visibility(["zohar"], true)
...
func _input(event):
	...
	if event is InputEventKey and event.pressed and not is_dragging:
		match event.keycode:
			KEY_1:
				play_animation("idel")
				set_node_visibility(["bulu", "lang"], false)
				set_node_visibility(["zohar"], true)
			KEY_2:
				play_animation("bulu")
				set_node_visibility(["zohar", "lang"], false)
				set_node_visibility(["bulu"], true)
			KEY_3:
				play_animation("lang")
				set_node_visibility(["zohar", "bulu"], false)
				set_node_visibility(["lang"], true)
