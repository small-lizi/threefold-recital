...
# 道具节点列表
var prop_nodes = ["table", "wc"]

# 添加道具默认翻转状态字典
var prop_default_flip = {
	"table": false,  # false表示默认不翻转
	"wc": true     # 可以根据实际需要设置每个道具的默认翻转状态
}
	...
	if event is InputEventKey and event.pressed and not is_dragging:
		match event.keycode:
			KEY_1:
				stop_movement()
				play_animation("idel")
				set_node_visibility(["table", "wc"], false)
			KEY_2:
				set_node_visibility(["table", "wc"], false)
				toggle_movement("run")
			KEY_3:
			...
			KEY_4:
				stop_movement()
				play_animation("sit")
				set_node_visibility(["wc"], true)
				set_node_visibility(["table"], false)
			KEY_5:
				stop_movement()
				play_animation("table")
				set_node_visibility(["table"], true)
				set_node_visibility(["wc"], false)
...
func _on_animation_finished():
	if animated_sprite.animation in ["table","punch"]:
		play_animation("idel")
...
func set_node_visibility(node_names: Array, is_visible: bool) -> void:
	...
	for target_node in found_nodes:
		...
		# 如果节点可见且是Sprite2D，设置翻转（考虑默认翻转状态）
		if is_visible and target_node is Sprite2D:
			# 获取道具的默认翻转状态，如果没有设置则默认为false
			var default_flip = prop_default_flip.get(target_node.name, false)
			# 根据当前方向和默认翻转状态计算最终的翻转状态
			target_node.flip_h = default_flip != (current_direction < 0)  # 使用异或逻辑
